  ...
  return (
    <div className={`flex items-start gap-3 p-4 border-b border-slate-50 transition-colors last:border-none ${notif.read ? "bg-white" : "bg-blue-50/50"}`}>
      ...
      <div className="flex-1 flex flex-col gap-1">
        <p className={`text-sm leading-snug ${notif.read ? "text-slate-600" : "text-slate-800 font-medium"}`}>
          {notif.message}
        </p>
        ...
        <div className="absolute right-0 mt-2 w-80 bg-white rounded-xl shadow-2xl border border-slate-100 overflow-hidden z-50">
          ...
          <div className="max-h-96 overflow-y-auto">
            {notifications.length === 0 ? (
              <div className="p-4 text-center text-sm text-slate-500">No tenés notificaciones.</div>
            ) : (
