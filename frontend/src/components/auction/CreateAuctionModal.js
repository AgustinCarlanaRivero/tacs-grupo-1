          ...
          {requirements.map(req => (
            <div key={req.sticker.number} className="flex items-center gap-3 bg-slate-50 border border-slate-200 rounded-xl px-3 py-2">
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                <button onClick={() => onChangeQty(req.sticker.number, -1)} className="w-6 h-6 rounded-full bg-slate-200 hover:bg-slate-300 flex items-center justify-center transition-colors">
                  <Minus size={10} />
                </button>
              ...
              <button onClick={() => onRemove(req.sticker.number)} className="ml-1 p-1 text-slate-400 hover:text-red-500 transition-colors shrink-0">
                <Trash2 size={14} />
              </button>
            </div>
          ))}
  ...
  return (
    ...
    >
      <div className="flex gap-1.5 px-4 pt-3 pb-1 shrink-0">
        {[1, 2, 3].map(s => (
          <div key={s} className={`h-1 flex-1 rounded-full transition-all ${s <= step ? "bg-[#002B5E]" : "bg-slate-200"}`} />
        ))}
      </div>

      <div className="flex-1 overflow-y-auto px-4 py-4 md:px-6 md:py-5">
        {step === 1 && <StepSticker myCollection={myCollection} selected={selectedSticker} onSelect={setSelectedSticker} />}
        {step === 2 && <StepRequirements catalog={myCollection} requirements={requirements} onAdd={addRequirement} onRemove={removeRequirement} onChangeQty={changeQty} />}
      ...
      <div className="shrink-0 p-4 md:p-6 border-t border-slate-100 flex gap-3">
        {step > 1 && (
          <button onClick={() => setStep(s => s - 1)} className="px-5 py-3 rounded-lg text-sm font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 transition-colors">
            Atrás
          </button>
        )}
        <button
