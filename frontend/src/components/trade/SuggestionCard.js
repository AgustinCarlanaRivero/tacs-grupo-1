...
function StickerSlot({ sticker, label, accent }) {
  return (
    <div className="flex flex-col items-center gap-1.5 flex-1 min-w-0">
      <span className={`text-[9px] font-bold uppercase tracking-widest ${accent ? "text-[#BF0A30]" : "text-slate-400"}`}>
        {label}
      </span>
      <div className={`relative rounded-xl overflow-hidden border-2 ${accent ? "border-[#BF0A30]/40" : "border-slate-200"}`}>
        <img
        ...
        <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/70 to-transparent px-1.5 py-1">
          <p className="text-white text-[10px] font-bold leading-tight text-center truncate">{sticker.player.name}</p>
          <p className="text-white/70 text-[8px] text-center truncate">{sticker.player.nationalTeam?.name}</p>
        </div>
      </div>
    </div>
  );
