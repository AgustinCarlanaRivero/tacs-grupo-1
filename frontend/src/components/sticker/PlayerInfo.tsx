...
import type { MockPlayer } from "@/data/types";

type Variant = "shiny" | "regular";

const STYLES: Record<Variant, { name: string; country: string; dot: string; club: string }> = {
  shiny: {
...
function InfoContent({ player, variant }: InfoContentProps) {
  ...
  return (
    <>
      <h3
        className={`text-sm font-black uppercase tracking-[0.12em] leading-tight break-words mb-1.5 ${s.name}`}
      >
        {player.name}
      </h3>
      <div
        className={`w-full h-px mb-1.5 ${variant === "shiny" ? "bg-white/20" : "bg-slate-200"}`}
      ></div>
      <div className="flex items-center gap-1.5">
        <span className={`text-[11px] font-bold uppercase tracking-wider ${s.country}`}>
          {country}
        </span>
        <span className={`text-[11px] ${s.dot}`}>•</span>
        <span className={`text-[11px] font-medium uppercase tracking-wider ${s.club}`}>
          {club}
        </span>
      </div>
    </>
...
export function PlayerInfoOverlay({ player, variant = "shiny" }: PlayerInfoProps) {
  return (
...
export function PlayerInfoPanel({ player, variant = "regular" }: PlayerInfoProps) {
  return (
