...
interface NumberBadgeProps {
  number: number;
  variant?: "regular" | "shiny";
}

export default function NumberBadge({ number, variant = "regular" }: NumberBadgeProps) {
  const isShiny = variant === "shiny";

  return (
    <div className="absolute top-2 left-2 z-20 flex flex-col items-start gap-1">
      <div
        className={`font-black text-sm px-2 py-0.5 shadow-md backdrop-blur-sm ${
          isShiny ? "bg-yellow-400/90 text-yellow-950" : "bg-white/90 text-slate-800"
        }`}
      >
        #{number}
      </div>
