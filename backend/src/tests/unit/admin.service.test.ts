...
describe("AdminService", () => {
    ...
    describe("getStats", () => {
        it("agrega usuarios por rol y notificaciones por tipo / estado", async () => {
            ...
            expect(
                stats.notifications.byType[NotificationType.OFFER_RECEIVED],
            ).toBe(2);
            expect(
                stats.notifications.byType[NotificationType.AUCTION_ENDING],
            ).toBe(1);
        });
    });
    ...
    describe("updateUserRole", () => {
        ...
        it("impide degradar al último admin", async () => {
            const adminId = await seedAdmin();
            const otherAdmin = await AuthService.getOrCreateUser(
                "auth0|other",
                { email: "o@x.com" },
            );
