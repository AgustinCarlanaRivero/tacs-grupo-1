...
export default class PostController {
    listPosts = async (req: Request, res: Response) => {
        const { type, state, query, page, limit } =
            req.query as unknown as PostFilterQuery;
    ...
    createPost = async (req: Request, res: Response) => {
        ...
        if (ownerId !== authUserId) {
            throw new ForbiddenError(
                "No autorizado para crear publicaciones en otro usuario",
            );
        }
    ...
    updatePostState = async (req: Request, res: Response) => {
        ...
        if (ownerId !== authUserId) {
            throw new ForbiddenError(
                "No autorizado para actualizar publicaciones de otro usuario",
            );
        }

        const { state } = req.body as UpdateStateBody;
        const updated = await PostService.updatePostState(
            ownerId,
            postId,
            state,
        );
